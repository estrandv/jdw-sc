...
impl SCClient {
     ...
     */
    pub fn send_timed_packets(&self, delay_ms: u64, msgs: Vec<TimedOSCPacket>) {

        for msg in msgs {
            if msg.time == BigDecimal::zero() {
                self.send_with_delay(msg.packet, delay_ms);
            } else {
                ...
                self.send_with_delay(msg.packet, delay_ms + time_integer);
            }
        }
    }
...
// TODO: Deprecated, use replacing structs and functions 
